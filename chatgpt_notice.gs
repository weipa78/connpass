...
const SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/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';

function fetchChatGPTEvents() {
  const now = new Date();
  const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const twelveHoursAgo = new Date(now.getTime() - 12 * 60 * 60 * 1000);
  const apiUrl = `https://connpass.com/api/v1/event/?keyword=ChatGPT&count=100`;
  ...
  // Filter events that were updated within the last 12 hours and have a start date on or after today
  ...
  let eventMessage = '直近12時間以内に更新されたイベントは以下の通りです。\n';
